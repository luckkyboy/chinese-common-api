...
function getPriceDataPrefix(): string {
  const priceBaseUrl = buildGithubRawUrl({
    repo: config.oilPrice.repo,
    branch: config.oilPrice.branch,
    path: `${config.oilPrice.dataRootPath}/`,
  });
  return `${priceBaseUrl}prices/`;
}

function getLatestPointerUrl(): string {
...
async function loadAll(): Promise<DataSnapshot> {
  const latestPointerUrl = getLatestPointerUrl();
  const latestPointer = await fetchJson<LatestPointerPayload>(latestPointerUrl);

  if (!latestPointer.latest) {
    throw new Error(`Invalid latest pointer: missing "latest" field from ${latestPointerUrl}`);
  }

  const latestUrl = `${getPriceDataPrefix()}${latestPointer.latest}`;
